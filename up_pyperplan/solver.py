...
class SolverImpl(unified_planning.solvers.Solver):
    ...
    def solve(self, problem: 'up.model.Problem',
        ...
        if solution is None:
            return up.plan.FinalReport(PlanStatus.UNSOLVABLE_PROVEN, None, self.name)
        for action_string in solution:
            actions.append(self._convert_string_to_action_instance(action_string.name, problem))
        return up.solvers.PlanGenerationResult(PlanStatus.SOLVED_SATISFICING, up.plan.SequentialPlan(actions), self.name)
