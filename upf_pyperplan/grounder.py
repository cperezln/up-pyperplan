...
def _get_fresh_name(new_problem: 'upf.model.Problem', name: str) -> str:
...
def rewrite_back_task(task: 'pyperplan.task.Task', original_problem: 'upf.model.Problem') -> Tuple['upf.model.Problem', Dict['upf.model.Action', Tuple['upf.model.Action', List['upf.model.FNode']]]]:
    ...
    grounded_problem = upf.model.Problem(task.name, original_problem.env)
    rewrite_back_map: Dict['upf.model.Action', Tuple['upf.model.Action', List['upf.model.FNode']]] = {}
    #map from names in the task domain to fluents of the grounded problem
    vars_to_fluent_map: Dict[str, 'upf.model.Fluent'] = {}
    for fact in task.facts:
        fluent = upf.model.Fluent(_get_fresh_name(grounded_problem, _change_notation(fact)))
    ...
    for operator in task.operators:
        new_action = upf.model.InstantaneousAction(_get_fresh_name(grounded_problem, _change_notation(operator.name)))
        ...
        parameters: List['upf.model.Object'] = []
